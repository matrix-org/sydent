...
class JoinTokenStore(object):
    ...
    def storeToken(self, medium, address, roomId, sender, token, originServer=None,
                   originId=None, commit=True):
        ...
        if originId and originServer:
            ...
            if int(originId) <= int(last_processed_id):
                logger.info("We have already seen token ID %s from %s. Ignoring.", originId,
                            originServer)
                return
        ...
        cur.execute("INSERT INTO invite_tokens"
                    ...
                    " VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    (medium, address, roomId, sender, token, int(time.time()), originServer,
                     originId))
        if commit:
    ...
    def storeEphemeralPublicKey(self, publicKey, persistenceTs=None, originServer=None,
                                originId=None, commit=True):
        ...
        if originId and originServer:
            ...
            if int(originId) <= int(last_processed_id):
                logger.info("We have already seen key ID %s from %s. Ignoring.", originId,
                            originServer)
                return
